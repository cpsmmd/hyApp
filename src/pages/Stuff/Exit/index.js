...
import {EXIT_STATUS, MY_PASS} from '../../../util/constants';
import {dealFail} from '../../../util/common';
import {getSupplierByName, getExitApply} from '../../../api/stuff';
...
export default function Exit(props) {
  ...
  const getLists = async (num, statue) => {
    ...
    // setLoading(true);
    try {
      const res = await getExitApply(parms);
      if (res.data.code === 200) {
        let list = res.data.data.list || [];
        setIsAll(list.length < limit);
        console.log('退场列表', list);
        settableData(state => {
          return [...state, ...list];
        });
      } else {
      ...
      <View key={item.name} style={styles.list_item}>
        <View style={{display: 'flex', flexDirection: 'row'}}>
          ...
          <View
            ...
            {item.isOperate && item.exitState === 2 && (
              <TouchableWithoutFeedback
                onPress={() => {
            ...
            {item.isOperate && item.exitState === 18 && (
              <TouchableWithoutFeedback
                onPress={() => {
                  navigationTo('confirm', item.id);
                }}>
                <Text style={styles.edit_btn}>编辑</Text>
              </TouchableWithoutFeedback>
            )}
            {item.isOperate && item.exitState === 1 && (
              <TouchableWithoutFeedback
                onPress={() => {
        ...
        <View
          ...
          <Text>
            审批状态：
            <Text style={styles.list_item_text}>
              <RenderStatus status={item.exitState} />
            </Text>
          </Text>
        </View>
      </View>
